...
def run_query(con, sf, query_id, query_spec, numThreads, results_file):
    ...
    duration = end - start
    results_file.write(f"DuckDB-{duckdb.__version__}\t{numThreads} threads\t{sf}\t{query_id}\t{duration:.4f}\t{result[0][0]}\n")
    results_file.flush()
...
con = duckdb.connect(database='duckdb/scratch/ldbc.duckdb', read_only=True)
